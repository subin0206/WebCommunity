...
import com.rlasb.admin.config.auth.dto.OAuthAttributes;
...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException{
        ...
        ///네이버 로그인, 구글 로그인 구분해줌.
        //user NameAttributeName -> OAuth2 로그인 진행 시 키가 되는 필드값. Primary Key와 같은 의미
        //구글 -> 기본적으로 코드(sub)를 지원하지만, 네이버.카카오 등은 기본 지원 x
        //네이버 로그인과 구글 로그인을 동시 지원할 때 사용
        //OAuthAttributes -> OAuth2UserService를 통해 가져온 OAuth2User의 attribute를 담을 클래스
        //이후 네이버 등 다른 소셜 로그인도 이 클래스 사용함.
